...
interface ConsentRequestModalProps extends ModalProps {
  onAddEvent: (event: React.MouseEvent) => void;
}

const ConsentRequestModal = (props: ConsentRequestModalProps) => {
  const { onClose, open, onAddEvent, ...rest } = props;
  return (
    <Modal open={open} onClose={onClose} backdrop="static" {...rest}>
      <Modal.Header>
        <Modal.Title>Create a Consent Request</Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Form fluid>
